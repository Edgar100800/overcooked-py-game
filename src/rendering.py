...
class Renderer:
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.window_scale = float(self.config.get("window_scale", 2.0))
    ...
    def maybe_render(self, env, timestep: int, joint_action=None, reward: float | None = None):
        ...
        if self.mode == "window":
            ...
            self._show_surface_in_window(surface, env=env, timestep=timestep)
            if self.save_gif:
    ...
    def _show_surface_in_window(self, surface, env=None, timestep: int | None = None):
        ...
        if self.window_scale > 0 and self.window_scale != 1.0:
            width, height = surface.get_size()
            scaled_size = (int(width * self.window_scale), int(height * self.window_scale))
            display_surface = pygame.transform.scale(surface, scaled_size)

        width, height = display_surface.get_size()
        if self._screen is None:
            self._screen = pygame.display.set_mode((width, height))

        pygame.display.set_caption(self._caption(env=env, timestep=timestep))
        ...
        self._screen.blit(display_surface, (0, 0))
        pygame.display.flip()

        if self._clock is not None and self.fps > 0:
            self._clock.tick(self.fps)

    def _caption(self, env=None, timestep: int | None = None) -> str:
        if env is None:
            return self.window_caption

        layout_name = getattr(getattr(env, "mdp", None), "layout_name", "nivel")
        horizon = getattr(env, "horizon", None)
        if timestep is None:
            timestep = getattr(getattr(env, "state", None), "timestep", None)

        progress = ""
        if timestep is not None and horizon:
            progress = f" | Paso {int(timestep)}/{int(horizon)}"

        return f"{self.window_caption} | Nivel: {layout_name}{progress}"
